...
private:
    ...
    std::vector<PathPlanner::RegionPath> path_layers_;
    ...
    ros::Publisher segmented_regions_vis_pub_;  // 分割区域可视化
    ...
    void publishSegmentedRegionsVisualization();// 分割区域可视化
...
ShipPainterNode::ShipPainterNode() : nh_private_("~") {
    ...
    //分割区域可视化发布者
    segmented_regions_vis_pub_ = nh_.advertise<visualization_msgs::MarkerArray>(
    "/visualization/segmented_regions", 10);
...
bool ShipPainterNode::loadParameters() {
    ...
    // 区域生长参数
    int k_search, min_size, max_size;
    double smoothness, curvature;

    nh_private_.param<int>("region_growing/k_search", k_search, 30);
    nh_private_.param<double>("region_growing/smoothness_threshold", smoothness, 7.0);
    nh_private_.param<double>("region_growing/curvature_threshold", curvature, 1.0);
    nh_private_.param<int>("region_growing/min_cluster_size", min_size, 200);
    nh_private_.param<int>("region_growing/max_cluster_size", max_size, 1000000);

    planner_config.region_growing_config.k_search = k_search;
    planner_config.region_growing_config.smoothness_threshold = smoothness;
    planner_config.region_growing_config.curvature_threshold = curvature;
    planner_config.region_growing_config.min_cluster_size = static_cast<size_t>(min_size);
    planner_config.region_growing_config.max_cluster_size = static_cast<size_t>(max_size);

    ROS_INFO("Region Growing Config:");
    ROS_INFO("  - K search: %d", k_search);
    ROS_INFO("  - Smoothness: %.1f deg", smoothness);
    ROS_INFO("  - Curvature: %.3f", curvature);
    ROS_INFO("  - Min/Max size: %d / %d", min_size, max_size);
...
bool ShipPainterNode::generateSprayPath() {
    ...
    if (path_layers_.empty()) {
        ROS_ERROR("No path layers generated");
            // ===== 临时调试：即使失败也保持运行 =====
            ROS_WARN("Keeping node alive for debugging...");
            ros::Rate rate(10);
            while (ros::ok()) {
                ros::spinOnce();
                rate.sleep();
    ...
    if (isWaypointReached()) {
        if (!flight_state_.waypoint_reached) {
            ...
            ROS_INFO("Reached waypoint %zu in layer %zu",
                     ...
                     flight_state_.current_region_idx + 1);
        }
...
geometry_msgs::PoseStamped ShipPainterNode::computeTargetPose() {
    ...
    if (flight_state_.current_region_idx >= path_layers_.size()) {
...
void ShipPainterNode::advanceToNextWaypoint() {
    const auto& current_region = path_layers_[flight_state_.current_region_idx];
    
    flight_state_.current_waypoint_idx++;
    flight_state_.waypoint_reached = false;
    flight_state_.total_waypoints_reached++;
    
    if (flight_state_.current_waypoint_idx >= current_region.waypoints.size()) {
        ROS_INFO("Completed layer %zu/%zu",
                 flight_state_.current_region_idx + 1, path_layers_.size());
        
        flight_state_.current_region_idx++;
...
PathPlanner::Waypoint ShipPainterNode::getCurrentWaypoint() {
    if (flight_state_.current_region_idx < path_layers_.size()) {
        const auto& layer = path_layers_[flight_state_.current_region_idx];
...
PathPlanner::Waypoint ShipPainterNode::getInterpolatedWaypoint(double alpha) {
    ...
    if (flight_state_.current_waypoint_idx + 1 < 
        path_layers_[flight_state_.current_region_idx].waypoints.size()) {
        
        PathPlanner::Waypoint next = 
            path_layers_[flight_state_.current_region_idx]
...
void ShipPainterNode::visualizationTimerCallback(const ros::TimerEvent& event) {
    ...
    publishSegmentedRegionsVisualization();// 分割区域可视化
}

//分割区域可视化
void ShipPainterNode::publishSegmentedRegionsVisualization() {
    auto segmented_regions = planner_.getSegmentedRegions();
    
    if (segmented_regions.empty()) {
        return;
    }
    
    visualization_msgs::MarkerArray marker_array;
    
    ROS_INFO_THROTTLE(5, "Publishing %zu segmented regions for visualization", 
                      segmented_regions.size());
    
    for (size_t i = 0; i < segmented_regions.size(); ++i) {
        const auto& region = segmented_regions[i];
        
        // 创建区域点云marker
        visualization_msgs::Marker region_marker;
        region_marker.header.frame_id = params_.frame_id;
        region_marker.header.stamp = ros::Time::now();
        region_marker.ns = "segmented_region";
        region_marker.id = i;
        region_marker.type = visualization_msgs::Marker::SPHERE_LIST;
        region_marker.action = visualization_msgs::Marker::ADD;
        
        // 点的大小
        region_marker.scale.x = 0.02;
        region_marker.scale.y = 0.02;
        region_marker.scale.z = 0.02;
        
        // 使用预分配的颜色
        region_marker.color.r = region.color_rgb.x();
        region_marker.color.g = region.color_rgb.y();
        region_marker.color.b = region.color_rgb.z();
        region_marker.color.a = 0.8;
        
        // 添加所有点（应用坐标变换）
        for (const auto& pt : region.cloud->points) {
            geometry_msgs::Point p;
            p.x = pt.x + params_.display_model_x;
            p.y = pt.y + params_.display_model_y;
            p.z = pt.z + params_.display_model_z;
        ...
        normal_marker.header = region_marker.header;
        normal_marker.ns = "region_normal";
        normal_marker.id = i + 1000;
        normal_marker.type = visualization_msgs::Marker::ARROW;
        normal_marker.action = visualization_msgs::Marker::ADD;
        
        // 箭头起点：区域质心
        geometry_msgs::Point start;
        start.x = region.centroid.x() + params_.display_model_x;
        start.y = region.centroid.y() + params_.display_model_y;
        start.z = region.centroid.z() + params_.display_model_z;
        
        // 箭头终点：沿法向量方向
        geometry_msgs::Point end;
        double arrow_length = 0.3;
        end.x = start.x + region.average_normal.x() * arrow_length;
        end.y = start.y + region.average_normal.y() * arrow_length;
        end.z = start.z + region.average_normal.z() * arrow_length;
        
        normal_marker.points.push_back(start);
        normal_marker.points.push_back(end);
        
        normal_marker.scale.x = 0.02;  // 箭杆直径
        normal_marker.scale.y = 0.04;  // 箭头直径
        normal_marker.scale.z = 0.05;  // 箭头长度
        
        // 法向量箭头用同色但更深
        normal_marker.color.r = region.color_rgb.x() * 0.7;
        normal_marker.color.g = region.color_rgb.y() * 0.7;
        normal_marker.color.b = region.color_rgb.z() * 0.7;
        ...
        text_marker.header = region_marker.header;
        ...
        text_marker.pose.position.z += 0.2;  // 稍微抬高
        ...
        ss << "R" << i << "\n" << region.point_count << " pts";
        text_marker.text = ss.str();
        
        marker_array.markers.push_back(text_marker);
    }
    
    segmented_regions_vis_pub_.publish(marker_array);
    
    static int debug_count = 0;
    if (++debug_count % 100 == 0) {
        ROS_INFO("Published segmented regions: %zu regions, %zu markers",
                 segmented_regions.size(), marker_array.markers.size());
    }
}
...
void ShipPainterNode::publishPathVisualization() {
    ...
    for (size_t layer_idx = 0; layer_idx < path_layers_.size(); ++layer_idx) {
        ...
        // 颜色编码
        if (layer_idx < flight_state_.current_region_idx) {
            // 已完成层 - 灰色
            path_marker.color.r = 0.3;
            path_marker.color.g = 0.3;
            path_marker.color.b = 0.3;
            path_marker.color.a = 0.5;
        } else if (layer_idx == flight_state_.current_region_idx) {
            // 当前层 - 绿色
...
void ShipPainterNode::publishStatusVisualization() {
    ...
    if (flight_state_.mission_complete) {
        ...
        ss << "Layer: " << (flight_state_.current_region_idx + 1) 
           << "/" << path_layers_.size() << "\n";
