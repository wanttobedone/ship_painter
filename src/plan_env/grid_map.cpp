...
void GridMap::initMap(ros::NodeHandle &nh)
{
  ...
  // Load cam2body extrinsic from parameter server (camera to body transform)
  std::vector<double> cam2body_vec;
  if (node_.getParam("grid_map/cam2body", cam2body_vec) && cam2body_vec.size() == 16) {
    // Fill 4x4 matrix from 1D array (row-major order)
    for (int i = 0; i < 4; ++i) {
      for (int j = 0; j < 4; ++j) {
        md_.cam2body_(i, j) = cam2body_vec[i * 4 + j];
      }
    }
    ROS_INFO("[GridMap] Loaded cam2body extrinsic from parameter server");
    ROS_INFO("[GridMap] Translation: [%.3f, %.3f, %.3f]",
             md_.cam2body_(0,3), md_.cam2body_(1,3), md_.cam2body_(2,3));
  } else {
    ROS_ERROR("[GridMap] Failed to load 'grid_map/cam2body' parameter!");
    ROS_ERROR("[GridMap] Expected 16 values (4x4 matrix in row-major order)");
    ROS_ERROR("[GridMap] Please check your launch file configuration");
    // Use identity as fallback to prevent crash
    md_.cam2body_ = Eigen::Matrix4d::Identity();
    ROS_WARN("[GridMap] Using identity matrix as fallback - mapping will be incorrect!");
  }
  ...
  indep_depth_sub_ =
      node_.subscribe<sensor_msgs::Image>("grid_map/depth", 10, &GridMap::depthCallback, this);
  ...
  map_marker_pub_ = node_.advertise<visualization_msgs::MarkerArray>("/grid_map/occupancy_marker", 10);
  map_inf_marker_pub_ = node_.advertise<visualization_msgs::MarkerArray>("/grid_map/occupancy_inflate_marker", 10);
...
void GridMap::projectDepthImage()
{
  ...
  // 动态调整缓冲区大小，防止数组越界
  int required_size = cols * rows / mp_.skip_pixel_ / mp_.skip_pixel_ + 1000; // 加1000作为安全余量
  if (md_.proj_points_.size() < required_size) {
    md_.proj_points_.resize(required_size);
    ROS_INFO_ONCE("[GridMap] Resized proj_points_ buffer to %d (image: %dx%d)", required_size, cols, rows);
  }
...
void GridMap::visCallback(const ros::TimerEvent & /*event*/)
{
  ...
  // 发布 MarkerArray 格式（立体小格子）
  publishMapMarker();
  publishMapInflateMarker(true);
}
...
{
  // 始终更新里程计位姿（用于独立的深度图订阅）
  // 从里程计获取body位姿，然后转换到相机坐标系
...
void GridMap::depthCallback(const sensor_msgs::ImageConstPtr &img)
{
  ...
  if (!md_.has_odom_)
  {
    // ROS_WARN_THROTTLE(5.0, "[GridMap] Received depth image but no odometry yet");
    return;
  }
  ...
  try {
    ...
    ROS_ERROR("[GridMap] cv_bridge exception: %s", e.what());
    return;
  }
...
void GridMap::publishMapMarker()
{
  if (map_marker_pub_.getNumSubscribers() <= 0)
    return;
  ...
  // 设置立方体大小
  marker.scale.x = mp_.resolution_;
  marker.scale.y = mp_.resolution_;
  marker.scale.z = mp_.resolution_;
...
void GridMap::publishMapInflateMarker(bool all_info)
{
  if (map_inf_marker_pub_.getNumSubscribers() <= 0)
    return;
  ...
  marker.action = visualization_msgs::Marker::ADD;
  marker.id = 0;
  marker.ns = "occupancy_inflate_map";
  ...
  marker_array.markers.push_back(marker);
  map_inf_marker_pub_.publish(marker_array);
}
